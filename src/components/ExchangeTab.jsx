...
function ExchangeTab() {
  ...
  useEffect(() => {
    ...
    fetch(
      `https://api.apilayer.com/currency_data/live?source=${currency}&currencies=${baseCurrency}`,
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => {
        setRates(result.quotes[`${currency}${baseCurrency}`]);
    ...
    <div>
      <div className="card">
        <input
          ...
          type="text"
          placeholder="amount"
          value={NumberBeautify(source, currency)}
        ...
        <p>{NumberBeautify(`${xrates.toFixed(2)}`, baseCurrency)}</p>
        {/* <p>{xrates.toFixed(2)}</p> */}
      </div>
    </div>
  );
}
